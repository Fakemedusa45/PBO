...
public class Posttest6 {
    ...
    if (tipe.equals("kamera")) {
        kamera kamera = cariKameraById(idBarang);
        if (kamera == null) {
            System.out.println("Kamera tidak ditemukan.");
            return;
        }
        if (!kamera.isStatus()) {
            System.out.println("Kamera sedang tidak tersedia.");
            return;
        }
    } else if (tipe.equals("aksesoris")) {
        aksesoris aksesoris = cariAksesorisById(idBarang);
        if (aksesoris == null) {
            System.out.println("Aksesoris tidak ditemukan.");
            return;
        }
        if (!aksesoris.isStatus()) {
            System.out.println("Aksesoris sedang tidak tersedia.");
            return;
        }
    }
    ...
    System.out.println("Total penyewaan yang telah diajukan: " + totalPenyewaan);
    ...
    public static final String inputString(String pesan) {
        System.out.print(pesan);
        return scanner.nextLine();
    }

    public static final String inputString(String pesan, String defaultValue) {
        System.out.print(pesan + " (" + defaultValue + "): ");
        String input = scanner.nextLine();
        return input.isEmpty() ? defaultValue : input;
    }
    
    public static kamera cariKameraById(int id) {
    ...
    public static aksesoris cariAksesorisById(int id) {
    for (aksesoris aksesoris : daftarAksesoris) {
        if (aksesoris.getId() == id) {
            return aksesoris;
        }
    }
    return null; // Jika tidak ditemukan
